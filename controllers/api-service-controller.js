...
function addService(req, res) {
    const { name, nameRus, price } = req.body;
    console.log(req.body);

    // ? Проверка введенных значений
    if (!checkData(name, nameRus, price)) {
        return res.status(400).json({ message: "Заполните все поля!" });
    }

    let service = new Service({ name, nameRus, price });
...
function updateService(req, res) {
    ...
    Service.findByIdAndUpdate(serviceId, { name, nameRus, price })
